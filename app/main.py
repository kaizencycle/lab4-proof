...
from .dual_writer import write_smoke_test_bundle
from .dual_writer_adapter import write_bundle
from .archive_endpoint import router as archive_router
...
class SmokeInput(BaseModel):
    date: str
    custodian: str = "Kaizen"
    lab: str = "Lab4: Research & Study"
    tests: Dict[str, Dict[str, str]]  # {"health":{"status":"pass","details":"..."}, ...}
    wins: str
    blocks: str = ""
    next_intent: str
...
def verify_day(date: str):
    """
    Verifies the day's ledger integrity by recomputing the Merkle root.
    Returns 409 if mismatch, 400 if files missing, 200 if OK.
    """
    files = today_files(date)
    if not (DATA_DIR / files["ledger"]).exists():
        raise HTTPException(status_code=404, detail="No ledger for this date")
...
# ──────────────────────────────────────────────────────────────────────────────
# SMOKE TEST DUAL WRITER
# ──────────────────────────────────────────────────────────────────────────────

@app.post("/smoke/save")
def save_smoke_bundle(payload: SmokeInput):
    """
    Saves smoke test results in both JSON and Markdown formats.
    Returns file paths and SHA-256 hashes for CommandLedgerII integration.
    """
    out = write_smoke_test_bundle(
        date=payload.date,
...
@app.post("/smoke/save-v2")
def save_smoke_bundle_v2(payload: SmokeInput):
    """
    Enhanced smoke test bundle with plug-and-play dual-writer adapter.
    Includes optional ZIP archiving with Merkle root guard.
    """
    bundle = write_bundle(
        date=payload.date,
        name="smoke-test",
        ...
        zip_after=True  # auto-archive day folder if Merkle root exists
    )

    # Provide clear archive status for CommandLedgerII
    if "zip_file" in bundle:
        archive_status = "Archive sealed successfully."
    else:
        # either we didn't ask to zip, or the guard blocked it
        # expose guard error if present
        archive_status = bundle.get(
            "zip_error",
    ...
    return {
        ...
        "md_file": bundle["md_file"],
        "md_sha256": bundle["md_sha256"],
        "archive_status": archive_status,
        "zip_file": bundle.get("zip_file")
    }