<#  heartbeat-gpg.ps1 — Daily Ledger Heartbeat + Logs + Rotation + Slack + ZIP + Checksums + GPG
    ...
    - Signs checksums.txt with GPG for cryptographic authorship
#>
...
function Zip-Day {
  param(
    ...
    [string]$OutDir,
    [string]$DataDir,
    [string]$SignerEmail = $env:LEDGER_SIGNER     # set this env var to your key's email
  )
  ...
  # 3) Sign checksums.txt (if signer available)
  $sigPath = $null
  if ($SignerEmail) {
    $sigPath = Sign-File -FilePath $checksumsPath -Signer $SignerEmail
  } else {
    Log "LEDGER_SIGNER not set; skipping GPG signature." Yellow
  }

  # 4) Zip folder (includes checksums + optional .sig)
...
function Verify-Checksums {
  ...
  if (-not (Test-Path $checks)) { Log "ZIP missing checksums.txt" Red; Remove-Item $tmp -Recurse -Force; return $false }

  # If a signature exists, verify it first
  $sig = Join-Path $tmp "checksums.txt.sig"
  if (Test-Path $sig) {
    if (-not (Verify-Signature -FilePath $checks -SigPath $sig)) {
      Log "GPG signature verification FAILED for checksums.txt" Red
      Remove-Item $tmp -Recurse -Force
      return $false
    } else {
      Log "GPG signature verification: OK" Green
    }
  } else {
    Log "checksums.txt.sig not found; skipping signature verify" Yellow
  }

  # Now verify each JSON hash matches checksums.txt
  $ok = $true
...
try {
  ...
  if ($zipPath) {
    ...
    if (Verify-Checksums -ZipPath $zipPath) {
      Log "ZIP checksum + signature verification: OK" Green
    } else {
      Log "ZIP checksum + signature verification: FAILED" Red
