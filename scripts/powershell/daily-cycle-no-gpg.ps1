...
try {
  ...
  # 7) copy files (without GPG signing/encryption)
  $TodayFiles = @(
  ...
  $summaryLines += "Artifacts: $($TodayFiles.Count) files copied (no GPG - install Gpg4win for signing/encryption)"
